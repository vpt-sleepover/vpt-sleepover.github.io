...
const editTemplate = (room, onSubmit) => html`
    ...
    <form @submit=${onSubmit}>
        ...
        <label>
            Location:
            <input type="text" name="location" .value=${room.locaiton}
        /></label>
        <label>
            Beds: <input type="number" name="beds" .value=${room.beds}
        /></label>
        <label>
            Open for booking
            <input
...
export function editView(ctx) {
    ctx.render(editTemplate(ctx.data, sumbitHandler(onSubmit)));
    ...
    async function onSubmit({ name, location, beds, openForBooking }) {
        ...
        await roomService.update(
            id,
            { name, location, beds, openForBooking },
            userId
        );
